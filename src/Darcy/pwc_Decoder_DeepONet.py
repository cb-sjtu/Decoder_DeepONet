...
def get_data(filename, ndata):
    ...
    # np.save('x_branch_una_train',x_branchs)
    # np.save('gridss_train',gridss)
    # np.save('y_unaligned_train',y_unaligned)
    x_branch = x_branch.reshape(ndata, s * s)
...
def main():
    ...
    losshistory, train_state = model.train(epochs=25000, batch_size=500,display_every=500,model_save_path="deeponet_AE_model_unaligned_noava/")
    print("\nTraining done ...\n")
    dde.saveplot(losshistory, train_state,issave=True,isplot=True)
    
    # model.restore("./deeponet_AE_model_unaligned_noava/-25000.ckpt")
    ...
    results=[]
    for k in range(200):
        spacename="./deeponet_AE_model_unaligned_noava/test_r15_"+str(k)+".dat"
