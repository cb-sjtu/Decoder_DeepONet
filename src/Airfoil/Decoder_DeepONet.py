...
from deepxde.nn.tensorflow_compat_v1.mionet import MIONetCartesianProd,MIONet_CNN,MIONet_CNN_no_average
    ...
    if problem == "ODE":
        ...
        branch_1 = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Conv2D(32, (3, 5), strides=2, activation="relu"),
                tf.keras.layers.Conv2D(16, (3, 5), strides=2, activation="relu"),
        ...
        trunk =  tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Conv2D(32, (3, 5), strides=3, activation="relu"),
                tf.keras.layers.Conv2D(16, (3, 5), strides=3, activation="relu"),
                tf.keras.layers.Flatten(),
                # tf.keras.layers.Dense(128, activation="relu"),
                tf.keras.layers.Dense(200),
                # tf.keras.layers.Dense(500),
        ...
        [   tf.keras.layers.InputLayer(input_shape=(200,2,)),
...
def get_data(N_points):
    ...
    X_train = (np.reshape(xy_branch_train[0],(1,512)),np.reshape(xy_train[0],(1,24100,2)) )
    X_test =  (np.reshape(xy_branch_test[0],(1,512)),np.reshape(xy_test[0],(1,24100,2)) )
    return X_train, np.reshape( label_train[0],(1,24100)), X_test,np.reshape( label_test[0],(1,24100)),data_test,xy_branch_test, label_test

def run(problem, lr, epochs, m, N_points,activation, initializer,datahome):
    ...
    data = Triple(X_train, y_train, X_test, y_test)
    net = MIONet_CNN_no_average(
        branch_net_1,
    ...
    checker = dde.callbacks.ModelCheckpoint(
        "./"+datahome+"/CNN_deeponet_no_average_model.ckpt", save_better_only=True, period=1000
    )
    
    losshistory, train_state = model.train(epochs=epochs ,display_every=100, callbacks=[checker],model_save_path="./"+datahome+"/")
    dde.saveplot(losshistory, train_state,issave=True,isplot=True)

    # model.restore("./CNN_deeponet_average/-250000.ckpt")
    ...
    for k in range(len(X_test[0])):
        ...
        np.savetxt("./"+datahome+"/shape"+str(k)+".dat",shape)
        np.savetxt("./"+datahome+"/test100_"+str(k)+".dat",result)
...
def main():
    ...
    datahome="CNN_deeponet_no_average"

    run(problem, lr, epochs, m,N_points, activation, initializer,datahome)
